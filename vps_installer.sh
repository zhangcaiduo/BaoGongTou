#!/bin/bash
#================================================================
# “    VPS 从零开始装修面板    ” v6.5.1 -    精装修 & 体验优化版
#    1.   视觉优化：主标题改为绿色，更醒目。
#    2.   主题重塑："白手起家搞装修"风格。
#    3.   排版对齐：优化主菜单的对齐方式，使其在各类终端中显示更美观。
#
...
#  获取脚本的绝对路径
if [[ "$0" == "bash" || "$0" == "sh" ]]; then
#  如果通过  "bash script.sh"  或管道执行，则无法直接获取路径，此功能将跳过
SCRIPT_PATH=""
else
...
#  仅当脚本路径有效且命令不存在或指向错误时才创建
if [[ -n "$SCRIPT_PATH" ]] && { [ ! -L "${LINK_PATH}" ] || [ "$(readlink -f ${LINK_PATH})" != "${SCRIPT_PATH}" ];
}; then
...
local text="$2"
local check_path="$3"
local status_info="$4" # Format: "type:details"
...
;;
docker_nopm)
formatted_details=" 容器 : ${details} ( 已接入总线 )"
;;
multi_docker)
...
;;
system)
formatted_details=" 系统服务 "
;;
system_port)
formatted_details=" 服务端口 : ${details}"
;;
rclone)
...
echo -e "${GREEN}============ VPS 从毛坯房开始装修VPS 包工头面板 v6.5.1 ============================================${NC}"
...
if [ -z "$MAIN_DOMAIN" ]; then echo -e "${RED}     错误：主域名不能为空！    ${NC}"; sleep 2;
return; fi
...
if [ -z "$WP_DOMAIN" ]; then echo -e "${RED}     错误：域名不能为空！    ${NC}"; sleep 2;
return; fi
...
if [ -z "$AI_DOMAIN" ]; then echo -e "${RED}     错误：    AI     域名不能为空！    ${NC}";
sleep 2; return; fi
clear
...
mkdir -p /root/alist_data /root/gitea_data /root/memos_data
...
[[ " ${components_to_install[@]} " =~ " qb " ]] && qb_status="${GREEN}[     已选     ]${NC}" ||
qb_status=""
[[ " ${components_to_install[@]} " =~ " jd " ]] && jd_status="${GREEN}[     已选     ]${NC}" ||
jd_status=""
[[ " ${components_to_install[@]} " =~ " yt " ]] && yt_status="${GREEN}[     已选     ]${NC}" ||
yt_status=""
...
if [ ${#components_to_install[@]} -eq 0 ]; then echo -e "${YELLOW}     您没有选择任何工具，操作取消。    ${NC}";
sleep 2; return;
fi
...
if [ -z "$NEW_USER" ]; then echo -e "${RED}     用户名不能为空，操作取消。    ${NC}"; sleep 2;
return; fi
...
# 11.     安装     AI     知识库
install_ai_model() {
if [ ! -d "/root/ai_stack" ]; then echo -e "${RED}     错误：AI 大脑未安装!${NC}";
sleep 3; return; fi
clear
...
if [ ! -d "/root/nextcloud_data" ]; then echo -e "${RED}     错误：Nextcloud 套件未安装!${NC}";
sleep 3; return;
fi
clear
...
if [ -z "$nc_domain" ]; then echo -e "${RED}     错误    :     无法从凭证文件找到     Nextcloud     域名    !${NC}";
sleep 3; return; fi
...
if [[ "$service_choice" == "b" ]]; then break; fi
...
echo -e "  - ${YELLOW}   当询问    'Use auto config?'
时，必须选    'n' (no)${NC}"
